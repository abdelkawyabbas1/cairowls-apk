...
// .figma/make/site.json is optional metadata used only inside the Figma Make
// authoring environment. It's read at Node config-eval time (not bundled),
// so the production/CI build no longer depends on that file existing.
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? 'Premium CRM'
